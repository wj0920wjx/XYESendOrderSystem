...
@implementation CharteredCarView

+(CharteredCarView *)initCharteredCarView{
    UINib *nib = [UINib nibWithNibName:@"CharteredCarView" bundle:nil];
    CharteredCarView *aView = [[nib instantiateWithOwner:self options:nil] lastObject];
    return aView;
}
...
-(void)awakeFromNib{
    ...
    self.chartWayTextView.wzb_placeholder = @"游玩行程";
    ...
    [[self.adultTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        ...
        [self.charteredDic setObject:self.adultTextField.text forKey:@"adult"];
    }];
    [[self.childTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        ...
        [self.charteredDic setObject:self.childTextField.text forKey:@"child"];
    }];
    [[self.luggageTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        ...
        [self.charteredDic setObject:self.luggageTextField.text forKey:@"luggage"];
    }];
    [[self.childSetTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        ...
        [self.charteredDic setObject:self.childSetTextField.text forKey:@"childSeat"];
    }];
    
    [[self.charteDayTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        @strongify(self)
        self.charteDayTextField.text = [NSString stringWithFormat:@"%ld",[x integerValue]];
        [self.charteredDic setObject:self.charteDayTextField.text forKey:@"car_day"];
    }];
    ...
    [[self.selectCarTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        ...
        [ZJPickerView zj_showWithDataList:self.carList propertyDict:nil completion:^(NSString * _Nullable selectContent) {
            ...
            [self.charteredDic setObject:self.selectCarTextField.text forKey:@"car"];
        }];
    }];
    
    [[self.prefixTextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        @strongify(self)
        [self.charteredDic setObject:self.prefixTextField.text forKey:@"car_suf"];
    }];
    ...
    [[[NSNotificationCenter defaultCenter]
     ...
     subscribeNext:^(NSNotification *notification) {
         if ([self.departTextView isFirstResponder]) {
             @strongify(self)
             [self.charteredDic setObject:self.departTextView.text forKey:@"depart_site"];
         }
         else if ([self.noteTextView isFirstResponder]) {
             @strongify(self)
             [self.charteredDic setObject:self.noteTextView.text forKey:@"order_notes"];
         }
         else if ([self.deliveTextView isFirstResponder]) {
             @strongify(self)
             [self.charteredDic setObject:self.deliveTextView.text forKey:@"delivered_site"];
         }
         else if ([self.chartWayTextView isFirstResponder]) {
             @strongify(self)
             [self.charteredDic setObject:self.chartWayTextView.text forKey:@"car_stroke"];
         }
         else{
...
- (IBAction)charatDayBtn:(UIButton *)sender {
    NSInteger count = [self.charteDayTextField.text integerValue];
    if (sender.tag == 1) {
        //减
        if (count <= 1) {
            count = 1;
        }
        self.charteDayTextField.text = [NSString stringWithFormat:@"%ld",--count];
    }
    else{
        self.charteDayTextField.text = [NSString stringWithFormat:@"%ld",++count];
    }
...
#pragma mark -- 懒加载
-(NSMutableDictionary *)charteredDic{
    if (!_charteredDic) {
        _charteredDic = [NSMutableDictionary dictionaryWithCapacity:0];
    }
    return _charteredDic;
}
